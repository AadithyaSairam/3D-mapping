# 3D Mapping: real-time monocular visual SLAM to Unity
...
1. Create a new Unity project with the **3D (Built-in Render Pipeline)**
   ...
   Built-in RP specifically, so see `docs/ARCHITECTURE.md` if you want to
   port it to URP.
...
map. This whole suite runs headless, with no webcam or Unity required. It
is useful both as a starting point if you extend the pipeline and as evidence
